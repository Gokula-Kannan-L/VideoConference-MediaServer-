...
const VideoTile:FunctionComponent<VideoTileProps> = ({
    ...
    const {pinRemoteVideo, pinScreenShare} = useGlobalState();
    ...
    return(
        <div className="videotile-container" style={{width, height}} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            ...
            {audioEnabled && <audio ref={audioRef} className="audio-element"  autoPlay playsInline></audio>}
            {
                isHovered &&
                <div className="video-pin">
                    {isPinned ? 
                        <div className="pin-icon" onClick={() => screenShareEnabled ? pinScreenShare() : pinRemoteVideo()}>
                            <FlashOff/> 
                        </div>
                        :
                        <div className="pin-icon" onClick={() => screenShareEnabled ? pinScreenShare() : pinRemoteVideo()}>
