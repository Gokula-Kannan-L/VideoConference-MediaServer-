...
const LocalTile: FunctionComponent = () => {
    const {meetStateRef, audio, video, localStream, pinVideo, participants} = useGlobalState();
        ...
        <div className="local-tile" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            ...
            {isHovered && <div className="video-pin">
                {meetStateRef.current?.currentUser.isPinned ? 
                    <div className="pin-icon" onClick={ () => pinVideo(false, String(meetStateRef.current?.currentUser.userKey), true)}>
                        <FlashOffIcon/> 
                    </div>
                    :
                    <div className="pin-icon" onClick={ () => pinVideo(true, String(meetStateRef.current?.currentUser.userKey), true)}>
                        <FlashOnIcon/>
                    </div>
                }
            </div>}
