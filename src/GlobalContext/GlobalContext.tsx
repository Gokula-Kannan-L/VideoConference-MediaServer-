...
type GlobalContextType = {
    ...
    pinVideo: (pin: boolean, userKey: string, pinnedStream: MediaStream, self?: boolean, screenshare?:boolean) => void
...
export const GlobalProvider = (props: {children: ReactNode}):ReactElement => {
    ...
    useEffect( () => {
        ...
        if(meetStateRef.current && meetStateRef.current?.host.hostId == meetStateRef.current?.currentUser.userId && isScreenSharing && displayStream){
            setMainTileInfo({
                message: "You're(Host) presenting the screen",
            ...
            if(participants){
                Object.keys(participants).forEach( (key) => {
                    if(key == meetStateRef.current?.host.hostKey){
                        participants[meetStateRef.current?.host.hostKey].isPinned.screen = true;
                    }else{
                        participants[key].isPinned.screen = false;
                    }
                })
            }
    
        }else if(participants && meetStateRef.current?.host.hostId != meetStateRef.current?.currentUser.userId ){
            let screenShareInfo: {
                message: string,
                stream: MediaStream,
                pinnedUser: null
            ...
            Object.keys(participants).forEach((key) => {
                ...
                if(host.userId == meetStateRef.current?.host.hostId && host.preference.screen && host.screenShare.displayStream){
                    screenShareInfo = {
                        message: `${participants[key].userName}(Host) is presenting the screen`,
                        stream: host.screenShare.displayStream,
                        pinnedUser: null
                    }
                    participants[key].isPinned.screen =true;
                }else{
                    participants[key].isPinned.screen =false;
                }
            });
    ...
    const pinVideo = (pin: boolean, userKey: string, pinnedStream: MediaStream, self?: boolean, screenshare?:boolean) => {
       
        if(self){
            if(pin && meetStateRef.current){
                if(screenshare)
                    meetStateRef.current.currentUser.isPinned = {screen: true, video: false};
                else
                    meetStateRef.current.currentUser.isPinned = {screen: false, video: true};
                ...
                if(meetStateRef.current){
                    meetStateRef.current.currentUser.isPinned = {screen: false, video: false};
                    setMainTileInfo(null);
                }
            ...
            Object.keys(participants).forEach((key) => {
                ...
                if(key == userKey){
                    if(screenshare)
                        ...
                        participants[key].isPinned.video = true;
                    if(user.preference.screen){
                        setMainTileInfo({
                    ...
                    participants[key].isPinned = {screen: false, video: false};
                }
            })
        }else if(!pin && participants?.[userKey]){
            setMainTileInfo(null);
            if(userKey == meetStateRef.current?.host.hostKey){
                participants[userKey].isPinned = {screen: false, video: false};
            }else{
                let update = {...participants};
                update[userKey].isPinned = {screen: false, video: false};
